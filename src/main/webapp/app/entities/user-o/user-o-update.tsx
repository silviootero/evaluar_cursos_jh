  ...
  return (
    <div>
      <Row className="justify-content-center">
        <Col md="8">
          <h2 id="evaluarCursosJhApp.userO.home.createOrEditLabel" data-cy="UserOCreateUpdateHeading">
            <Translate contentKey="evaluarCursosJhApp.userO.home.createOrEditLabel">Create or edit a UserO</Translate>
          </h2>
        </Col>
      </Row>
      <Row className="justify-content-center">
            ...
            <ValidatedForm defaultValues={defaultValues()} onSubmit={saveEntity}>
              {!isNew ? (
                <ValidatedField
                  ...
                  readOnly
                  id="user-o-userName"
                  label={translate('evaluarCursosJhApp.userO.userName')}
                  validate={{ required: true }}
                />
              ) : null}
              <ValidatedField
                label={translate('evaluarCursosJhApp.userO.password')}
                id="user-o-password"
                name="password"
                data-cy="password"
                type="text"
              />
              <ValidatedField
                label={translate('evaluarCursosJhApp.userO.email')}
                id="user-o-email"
                name="email"
                data-cy="email"
                type="text"
                validate={{}}
              />
              <ValidatedField id="user-o-role" name="role" data-cy="role" label={translate('evaluarCursosJhApp.userO.role')} type="select">
                <option value="" key="0" />
                {roles
